...
export let walkTokens = (token, m) => {
  if (token.type === "blockquote" && (m = token.text.match(/^\[!(NOTE|IMPORTANT|WARNING)\]\n/i))) {
    if (token.tokens[0] && token.tokens[0].type === "paragraph" && token.tokens[0].tokens.length) {
      ...
      token.tokens[0].tokens.unshift({ type: "html", tokens: void 0, text: icon, raw: icon });
    }
  }
};
